...
The server SHALL add a `shield` pickup kind. When a player collects a `shield` pickup, the server SHALL set a server-side `shieldMs` timer to `SHIELD_DURATION_MS`. While `shieldMs > 0`, the player SHALL NOT take damage from enemy bullets or enemy contact. The effect SHALL expire when `shieldMs` reaches zero.
...
The client SHALL render distinct sprites for each `PickupKind` value (`weapon`, `bomb`, `shield`, `rapid_fire`) using the positions provided in `SnapshotState.pickups`.
...
The client SHALL display a visible HUD badge or icon for each active timed effect (`shieldActive`, `rapidFireActive`) on the local player's HUD area. The badge SHALL disappear when the effect becomes inactive.
