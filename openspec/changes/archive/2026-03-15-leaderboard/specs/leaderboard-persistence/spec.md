...
The server SHALL provide an `InMemoryLeaderboardRepository` that implements the same interface as `RedisLeaderboardRepository`. When no `REDIS_URL` is configured, the server SHALL use the in-memory implementation.
...
- **AND** leaderboard operations (submit, query) function identically to the Redis implementation
